...
export const mercadoLivreCollector: SourceCollector = {
  async collect(source) {
    ...
    if (result.totalCollected === 0 && result.errors.length) throw new Error(result.errors[0]);
    return { itemsFound: result.totalCollected, itemsCreated: result.totalNew, itemsUpdated: null, itemsIgnored: Math.max(0, result.totalCollected - result.totalNew), errors: result.errors.length };
  },
};
